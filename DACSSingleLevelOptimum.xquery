...
declare variable $COLLECTION as document-node()* := collection("file:///path/to/findingaids/tamwag/?recurse=yes;select=*.xml");
...
$prefCite := exists($i//ead:archdesc/ead:prefercite),
$procInfo := exists($i//ead:archdesc/ead:processinfo),
$date := exists($i//ead:archdesc/ead:did/ead:unitdate)
return
<doc>
<file>{$doc}</file>
...
<prefercite>{$prefCite}</prefercite>
<processinfo>{$procInfo}</processinfo>
